...
import { createInvoiceDocs, createManyInvoicesDocs, downloadInvoiceDocs } from "./invoice.router.docs";


export async function invoiceRoutes(fastify: FastifyInstance) {
    fastify.post<{ Body: InvoiceCreate }>(
        '/',
        createInvoiceDocs,
        async (req, reply) => {
    ...
    fastify.post<{ Body: { installationId: string, invoices: InvoiceCreate[] } }>(
        '/create-many',
        createManyInvoicesDocs,
        async (req, reply) => {
